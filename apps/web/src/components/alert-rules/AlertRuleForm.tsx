...
function initFormFromApi(data: AlertRuleFormInitialData): { form: FormData; slackAlreadySet: boolean; teamsAlreadySet: boolean } {
  const slackMasked = isMasked(data.slackWebhookUrl)
  const teamsMasked = isMasked(data.teamsWebhookUrl)
  return {
    form: {
      name: data.name ?? '',
      status: data.status ?? 'ACTIVE',
      surveyTypes: data.surveyTypes ?? [],
      scoreMin: data.scoreMin != null ? String(data.scoreMin) : '',
      scoreMax: data.scoreMax != null ? String(data.scoreMax) : '',
      sentimentThreshold: data.sentimentThreshold != null ? String(data.sentimentThreshold) : '',
      topicFilters: (data.topicFilters ?? []).join(', '),
      // Leave blank if masked — the user must enter a new URL to change it
      slackWebhookUrl: slackMasked ? '' : (data.slackWebhookUrl ?? ''),
      slackChannelName: data.slackChannelName ?? '',
      emailRecipients: (data.emailRecipients ?? []).join('\n'),
      // Leave blank if masked
      teamsWebhookUrl: teamsMasked ? '' : (data.teamsWebhookUrl ?? ''),
      defaultAssignee: data.defaultAssignee ?? '',
      assignmentRules: Array.isArray(data.assignmentRules) ? data.assignmentRules : [],
      slaHours: data.slaHours != null ? String(data.slaHours) : '24',
    },
    slackAlreadySet: slackMasked,
    teamsAlreadySet: teamsMasked,
  }
}
...
export function AlertRuleForm({ mode, ruleId, initialData }: AlertRuleFormProps) {
  ...
  const [form, setForm] = useState<FormData>(() => {
    if (initialData) return initFormFromApi(initialData).form
    return EMPTY_FORM
  })
  const [slackAlreadySet, setSlackAlreadySet] = useState<boolean>(() =>
    initialData ? initFormFromApi(initialData).slackAlreadySet : false
  )
  const [teamsAlreadySet, setTeamsAlreadySet] = useState<boolean>(() =>
    initialData ? initFormFromApi(initialData).teamsAlreadySet : false
  )
  const [submitting, setSubmitting] = useState(false)
  const [serverError, setServerError] = useState<string | null>(null)
  const [errors, setErrors] = useState<Record<string, string>>({})

  // Keep the form in sync if initialData changes (e.g., parent re-fetches)
  useEffect(() => {
    if (initialData) {
      const initialized = initFormFromApi(initialData)
      setForm(initialized.form)
      setSlackAlreadySet(initialized.slackAlreadySet)
      setTeamsAlreadySet(initialized.teamsAlreadySet)
    }
  }, [initialData])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === 'edit') {
        ...
        payload.emailRecipients = emailList

        // Webhook URL rules:
        // - Typed a new value → send it
        // - Blank AND a masked value was already set → omit (preserve existing)
        // - Blank AND no value was set → send null (clear)
        ...
        // create: include webhooks + email only if set
        if (form.slackWebhookUrl) payload.slackWebhookUrl = form.slackWebhookUrl
        if (emailList.length > 0) payload.emailRecipients = emailList
        if (form.teamsWebhookUrl) payload.teamsWebhookUrl = form.teamsWebhookUrl
      }
      ...
      const url = mode === 'edit' ? `${API_URL}/v1/alert-rules/${ruleId}` : `${API_URL}/v1/alert-rules`
      ...
      if (!res.ok) {
        ...
        throw new Error((data as { message?: string })?.message ?? `Failed with status ${res.status}`)
      }
  ...
  const heading = mode === 'create' ? 'Create Alert Rule' : mode === 'edit' ? 'Edit Alert Rule' : 'Alert Rule'
  const subheading =
    mode === 'create'
      ? 'Configure when and how to alert your team about feedback'
      : mode === 'edit'
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  disabled={isViewOnly}
                  className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-gray-50 disabled:text-gray-500 ${errors.name ? 'border-red-400' : 'border-gray-300'}`}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      disabled={isViewOnly}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors disabled:opacity-60 ${form.status === 'ACTIVE' ? 'bg-indigo-600' : 'bg-gray-300'}`}
                    >
                      <span
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    disabled={isViewOnly}
                    className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-gray-50 disabled:text-gray-500"
                    placeholder="0"
                  />
                </div>
                <div>
                  ...
                  {form.assignmentRules.map((rule, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <input
                        ...
                        disabled={isViewOnly}
                        className="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-gray-50 disabled:text-gray-500"
          ...
          {!isViewOnly && (
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              >
                {submitting ? 'Saving…' : mode === 'edit' ? 'Save Changes' : 'Save Alert Rule'}
              </button>
            </div>
          )}
        </form>
