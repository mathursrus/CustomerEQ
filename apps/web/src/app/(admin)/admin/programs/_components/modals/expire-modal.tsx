    ...
    <Modal onClose={onClose} title="Retire Reward" size="md"
      footer={
        <div className="flex justify-end gap-3">
          ...
          <button type="button" onClick={handleConfirm} disabled={option === 'DATE' && !date} className="rounded-lg bg-red-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Confirm Retire</button>
        </div>
      }
    >
      <div className="space-y-4">
        ...
        {option === 'DATE' && (
          <div>
            ...
            <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            <p className="mt-1 text-xs text-gray-400">Reward will stop accepting new redemptions after this date.</p>
          </div>
        )}
      </div>
    </Modal>
