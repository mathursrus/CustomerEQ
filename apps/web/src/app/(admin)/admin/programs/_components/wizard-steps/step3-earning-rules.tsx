...
export function Step3EarningRules({
  ...
  const [noRuleError, setNoRuleError] = useState(false)

  // Pre-fill default rule if empty — guard with ref to prevent StrictMode double-add
  const defaultRuleAdded = useRef(false)
  useEffect(() => {
    if (!defaultRuleAdded.current && state.earningRules.length === 0) {
  ...
  return (
    <div>
      ...
      {noRuleError && (
        <p className="mt-3 text-sm text-red-500">At least one earning rule is required before advancing.</p>
      )}
