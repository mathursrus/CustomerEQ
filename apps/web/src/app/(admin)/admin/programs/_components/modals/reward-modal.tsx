    ...
    <Modal onClose={onClose} title={initialReward ? 'Edit Reward' : 'Add Reward'} size="lg"
      footer={
        <div className="flex justify-end gap-3">
          ...
          <button type="button" onClick={() => { onSave({ id: initialReward?.id ?? crypto.randomUUID(), ...form }); onClose() }} disabled={!form.name.trim() || !form.pointsCost.trim()} className="rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Save Reward</button>
        </div>
      }
    >
      <div className="space-y-4">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1">
            <label className="block text-xs font-medium text-gray-600 mb-1">Eligible Tiers</label>
