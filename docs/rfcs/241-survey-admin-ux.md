...
| `/v1/public/surveys/:id/respond` | POST | Same emission + policy semantics as the auth path. The prefill body fields (`memberId`, `email`, `phone`, `firstName`, `lastName`, `externalId`) reach `resolveOrEnrollMember(...)` unchanged (verified at `apps/api/src/routes/public.ts:295`). **URL `?email=` / `?member_id=` params no longer wired** — query-string identifier extraction is removed from the page handler per D51; #209's privacy concern is fully addressed. **`enrolledVia` channel attribution** changes accordingly: with the URL-query path gone, attribution derives from the body's existing `channel` field instead — `channel === 'in_app'` → `MemberEnrolledVia.EMBEDDED_FORM` (host application embedded the survey via the widget), anything else (`link`/`email`/`sms`/default) → `MemberEnrolledVia.SURVEY_RESPONSE` (standalone survey page; responder self-identified on the form). The embed widget already hardcodes `channel: 'in_app'` in its internal POST body (verified at `public.ts:798` today; same convention preserved in the Slice 5 Web Component); standalone defaults to `'link'`. Brand-facing integration is unchanged — `channel` is a widget-internal field, not a brand-passed parameter. | R15 / R16 / NFR-S4 |
