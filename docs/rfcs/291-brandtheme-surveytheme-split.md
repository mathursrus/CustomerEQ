...
The two "missing from architecture" entries (hand-edited migrations, backfill-before-drop ordering) are real patterns the team uses but doesn't document. **Resolved in this PR** ([r3199895853](https://github.com/mathursrus/CustomerEQ/pull/295#discussion_r3199895853): reviewer agreed to apply the doc edit on this branch rather than filing a separate issue): `docs/architecture/architecture.md` §3.4 Data Layer gains a fourth bullet describing the `--create-only` flow, the `ADD → BACKFILL → DROP` ordering, and the in-tree reference examples (`20260430000000_patch_survey_distribution_gap` for partial-migration recovery; this issue's `<timestamp>_brandtheme_surveytheme_split` for the rename + backfill shape). The third candidate ("public survey API select-set as a versioned contract") remains deferred — single data point, capture for a future architecture-gap retrospective if it recurs.
