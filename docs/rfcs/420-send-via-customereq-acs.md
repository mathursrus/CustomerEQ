...
- **Mode-parameterized React page component** (`<DistributePage mode={...}/>`) — new pattern. **Status**: lifted into `apps/web/src/components/mode-router/` as `<ModeRouter>` + `useModeRouter()` per reviewer r3292073383 / r3292074338 ("Lift it now. Don't punt architectural shortcuts based on 1st usage … not as a follow-up issue, but as an end of the feature implementation"). The primitive owns URL `?mode=` parsing, default-mode fallback (R5: absent `?mode=` falls through to a registered default), `switchTo(target)` helper that drops the param when target equals the default (clean bookmarkable URL), and a React context for child flows to call `useModeRouter().switchTo(...)`. Tests: `ModeRouter.test.tsx` (9 cases). First consumer: `/admin/surveys/[id]/distribute` (`self-serve` | `managed-email`). **Closest existing precedent in the codebase**: `/admin/programs/new/page.tsx` calls `<ProgramWizard mode="create" />` — that's a **component-level** mode prop on a single wizard, not a router-level dispatcher. #420's `<ModeRouter>` is the first router-level mode-parameterization in this codebase, lifted now (not after a second consumer surfaces) to honor the *no-shortcut-architecture* principle. Other surfaces likely to adopt the same router-level shape once they need divergent flows behind one URL: any future "draft preview vs live" workflow on tenant resources (themes, programs, surveys, campaigns). The earlier RFC draft cited `/admin/surveys/new` vs `/admin/surveys/[id]/edit` and `/admin/campaigns/[id]/preview` vs the live campaign route as additional candidates; that was factually wrong (per reviewer r3292070992 — verified): `/admin/surveys/new` is a Server Component that calls auth → POST `/v1/surveys` → `redirect('/admin/surveys/{newId}/edit?tab=basics')`, so it's a side-effectful create-and-redirect endpoint, not a peer of `/edit`; and `/admin/campaigns/[id]/preview` is a canvas-based spin-wheel preview page, not a mode-variant of the live campaign page. **An entry naming `<ModeRouter>` lands in `docs/architecture/architecture.md` §3.1 as part of `implement-architecture-update` (Phase 10) on this same PR.**
- **Polling-based progress UI** — frontend `useEffect` polling pattern is implicit in `LoopMonitor.tsx` (60s interval per verified read at `apps/web/src/components/surveys/LoopMonitor.tsx:92`). #420's 2s-interval polling for the Sending state is the same primitive at a faster cadence. With #420 the count of consumers becomes 2 (Loop Monitor + Managed-email Sending state); the lift into a shared `usePollingQuery` hook ships alongside the Loop Monitor stat-card extension covered later in this RFC so both call sites land on the shared hook in this PR (no follow-up issue).
- **Two-gate compliance suppression model** — audience-builder gate (UI) + worker pre-dispatch re-check. New pattern with clear V1 utility: any future workflow that selects then dispatches across a time-gap (e.g., scheduled-send if D-Non-goal "scheduled send" is reconsidered; campaign event triggers) will need the same two-gate shape. The pattern lands in `docs/architecture/architecture.md` §6 (Compliance) as part of `implement-architecture-update` (Phase 10) on this same PR — not as a follow-up issue.
