...
2. **Almost auto-merged PR #201 instead of waiting for explicit user direction**: Per `feedback_push_pr_always_merge_with_review.md`, merges need explicit user review on GitHub. Did not auto-merge despite both pipelines being green; waited for "merge it" before squash-merging.

## What I Did Wrong That The User Caught

1. **Recommended the sunk-cost answer first, hid the clean-slate answer behind a follow-up prompt**: On the JTBD spec re-segmentation discussion, my first response framed a "light reframe" (Option A — preserve the existing enum, three sub-issues, and three-bucket structure; repurpose #172 as a SaaS-connector hub) as the recommended path, with the framing explicitly anchored on "PR #197 shipped the enum, sub-issues exist." Only when the user asked, *"If we don't worry about sunk cost, what would you suggest?"*, did I pivot to the JTBD-based segmentation (winback / listen / reward), which was a substantively different and arguably better recommendation. The user's review on this retrospective PR #222 (line 84 of the original draft) flagged this directly: *"This is a key learning moment. When presenting options, we should consider both 'with sunk cost' and without sunk cost."* The earlier-draft entry in this retrospective categorized this as "almost did wrong but caught" — but I did not catch it; the user did, by asking a follow-up that explicitly stripped sunk cost out of the framing. The correct retrospective stance is that I shipped a single-frame recommendation that silently weighted "minimize churn" over "give the right answer" without naming the tradeoff. Captured durably in `feedback_present_both_sunk_cost_frames_upfront.md` (memory entry, indexed in `MEMORY.md`) and as a coaching moment at `fraim/personalized-employee/learnings/raw/manohar.madhira@outlook.com-2026-05-01T00-16-49-options-with-and-without-sunk-cost.md`.
...
8. **Strategic recommendations need both sunk-cost and clean-slate frames presented upfront** — when the user asks "is X the right approach?", the agent has implicit knowledge of in-flight work that the user (asking a fresh question) does not share. Defaulting to a "respect existing work" recommendation without surfacing the clean-slate alternative is a single-frame failure that buries the actual answer behind a follow-up prompt. Both frames belong in the same response, with the deciding tradeoff named explicitly. Captured durably in `feedback_present_both_sunk_cost_frames_upfront.md`. This lesson came from reviewer feedback on this retrospective itself — was *not* self-caught — and that fact has been corrected in the retrospective's "What I Did Wrong That The User Caught" section.
...
3. **`feedback_present_both_sunk_cost_frames_upfront.md`** — Saved during retrospective Round 1 feedback. Rule: on strategic / design / scoping questions, surface both "respect sunk cost" and "clean slate" recommendations side-by-side from the start; name the deciding tradeoff explicitly; let the user pick the frame. Includes trigger-question shapes and avoidance failure modes. Indexed in `MEMORY.md`.

All three memories include `Why:` (incident reference) and `How to apply:` (concrete checklist) per the feedback memory style guide.
