...
| **Cache/Queue** | Redis 7 + BullMQ v5 | Job queues with exactly-once semantics, idempotency keys, campaign deduplication |
...
- **Auth**: Clerk middleware guards all non-public routes; `ClerkProvider` wraps the app layout
...
- **Idempotency**: Redis-backed idempotency keys with 24-hour TTL on event ingestion. Redis SET NX for campaign trigger deduplication (one trigger per member per campaign). Database-level idempotency key stored on `LoyaltyEvent` for audit.
