...
- **Two-gate compliance suppression model** *(Issue #420)*: For any workflow that selects an audience at one moment and dispatches across a time-gap (managed-email send today; scheduled-send / event-triggered campaigns tomorrow), suppression is enforced at TWO independent gates: **Gate 1 (UI / audience-builder)** surfaces suppressed members visibly with a Status chip + disabled checkbox so the operator's intent excludes them; **Gate 2 (worker pre-dispatch)** re-checks suppression at the actual provider-call moment to catch the time-gap edge case (member unsubscribed between selection and dispatch). The worker gate is structurally enforced — `checkSuppression(member: { erased, unsubscribedSurveysAt, consentGivenAt, email })` deliberately does NOT accept `emailOptIn` as input, because surveys are legitimate-interest exempt from the marketing-channel opt-out. Canonical implementation: `apps/worker/src/processors/managedEmailSend.ts:checkSuppression`. Reuse pattern for any future select-then-dispatch workflow.
