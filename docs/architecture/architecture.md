...
- **`<ModeRouter>` URL-driven mode primitive**: When a single route surfaces ≥2 mode-parameterized flows (each with its own composer/CTA/state machine), use `apps/web/src/components/mode-router/` rather than inlining a `mode` `useState` with conditional JSX. The primitive takes a `modes` record (`{ [modeKey]: Component }`) plus a `defaultMode`; it reads/writes `?mode=` via Next's `useSearchParams` / `useRouter`, falls back to `defaultMode` when the param is missing or names an unknown key, and exposes `useModeRouter().{currentMode, switchTo}` to nested components for in-flow mode switching. Reference implementation: `apps/web/src/app/(admin)/admin/surveys/[id]/distribute/page.tsx` — `SELF_SERVE` vs `MANAGED_EMAIL` send modes (Issue #420). The standard CRUD `mode: 'create' | 'edit' | 'view'` prop pattern above is a *static* mode chosen by route, while `<ModeRouter>` is a *dynamic* mode the operator can switch at runtime via `?mode=` — both live in the catalog as distinct tools.
- **`usePollingQuery` hook**: Shared `useEffect`-based fetch-polling pattern for any UI that needs to refresh server state on a fixed cadence. `apps/web/src/lib/hooks/usePollingQuery.ts` accepts `{ fetchFn, intervalMs, enabled }` and returns `{ data, loading, error, refetch }`. Cancels in-flight fetches on unmount and on `enabled` flipping false, captures the latest `fetchFn` via ref so callers can pass inline arrows without restarting the interval, and supports manual `refetch()` for "Refresh now" affordances. Reference consumers: `apps/web/src/app/(admin)/admin/surveys/[id]/distribute/_components/ManagedEmailFlow.tsx` (2s `/send-progress` poll during MANAGED_EMAIL Sending state) and `apps/web/src/components/surveys/LoopMonitor.tsx` (60s `/loop-monitor` refresh when survey is ACTIVE). Add a new consumer here when introducing fixed-cadence polling — do not reimplement `setInterval` + cancellation per call site (Issue #420 R-lift).
...
- **Queue mode**: The worker process exists to drain BullMQ queues and is only deployed when `QUEUE_MODE=redis`. In `QUEUE_MODE=inline`, the same processor logic is invoked in-process from the API via `apps/api/src/queues/bullmq.ts` — the worker is not needed and is not run. Both modes execute the **same** processor functions and produce the **same** outcomes; Redis only changes scheduling and concurrency.
  - **`managed-email-send` honors this invariant via a Job-free core** *(Issue #420, commit `bf322ea`)*. The worker processor `processManagedEmailSend(job)` is a thin BullMQ adapter around `dispatchManagedEmailSend(payload, attemptInfo)` — the Job-free core in `apps/worker/src/processors/managedEmailSend.ts`. In `QUEUE_MODE=redis` the worker's BullMQ adapter calls it; in `QUEUE_MODE=inline` the API's `enqueueManagedEmailSend` calls the same core via `scheduleInline(...)` (attempts 3 + backoff 2000ms, matching the BullMQ config). This was a correction: the inline branch was previously a no-op that logged a warning, silently violating the inline≡redis invariant for this one queue. The core is shared across the package boundary via the `@customerEQ/worker/processors/managedEmailSend` subpath export — a deliberate **api → worker** import direction (no cycle: the worker has no dependency on the API). This is the only place the API imports from the worker package; it exists because the dispatcher logic is heavy (Prisma + connectors + render pipeline) and duplicating it in the API would risk drift.
...
| `managed-email-send` | `processManagedEmailSend` | 5 | **Issue #420** — per-recipient ACS dispatch for `DistributionBatch.sendMode = MANAGED_EMAIL`. Pre-dispatch **two-gate suppression re-check** (`Member.erased`, `unsubscribedSurveysAt`, `consentGivenAt`, `email`) — `emailOptIn` is **NOT** checked here (surveys are legitimate-interest, exempt from marketing opt-out — see §6 Two-gate Suppression Model). Renders email via `packages/shared/src/email/renderTemplate.ts` (inline-style, table-based layout — see §6 Email Rendering Pattern). Calls `sendEmailMessage` with the per-batch `opts.senderAddress` override. On success: UPDATE `SurveyDistribution.deliveredAt` + INCREMENT `Survey.sentCount`. On failure: classify into bounded `failureReason` (`bounce`/`invalid_address`/`transient_error_after_retries`/`skipped_*`). Writes `managed_email.send_attempt` AuditEvent. Concurrency 5 grounded in V0 expected send volume (50–500 recipients/batch × ~200ms ACS latency × ACS rate limits). **Payload carries plaintext tokens** *(Issue #420, commit `c4d779e`)*: `ManagedEmailSendPayload` includes `surveyLinkToken` + `unsubscribeToken` — a deliberate, bounded exception to the tokens-are-hash-at-rest rule. Distribution + unsubscribe tokens are SHA-256-hashed in Postgres (`tokenHash`), so the only moment plaintext exists is the in-memory mint at `POST /distribution-batches`. The worker needs the full plaintext to build valid recipient URLs (the public `/survey/:id/r/:token` route validates the full token, not the 8-char `tokenPrefix`), so the plaintext is threaded through the queue payload (transient in Redis/in-process, ~100 bytes/row) rather than re-derived. Retry-failed enqueues pass empty tokens and fall back to the prefix-based URL since the original plaintext is unrecoverable post-mint by design. |
...
- **Mode-parameterized React page component** *(Issue #420)*: When a single admin URL (`/admin/<resource>/<id>/<verb>`) needs to render distinct UX flows that share most state and primitives, parameterize via a `?mode=<discriminator>` URL query. The route's top-level page reads `searchParams.get('mode')` once and branches into separate flow components, each of which owns its own state via `useState`/`useEffect` hooks. Sharing state across the branch is avoided — the cost of duplicating shared inputs is lower than the cost of lifting all state into a wrapper context. Used in `apps/web/src/app/(admin)/admin/surveys/[id]/distribute/page.tsx` (#378 SelfServeFlow vs #420 MANAGED_EMAIL ManagedEmailFlow). Bookmarked-URL backward compat is preserved by defaulting absent / unknown modes to the legacy flow. **Hooks-rules compliance**: the discriminator branch must be the very first statement in the page component (before any `useState`/`useEffect`), routing to sibling components whose hooks lists are stable.

- **Two-gate compliance suppression model** *(Issue #420; spec §13.7, R22 + R23 + R43 + R44)*: For any workflow that selects an audience at one moment and dispatches across a time-gap (managed-email send today; scheduled-send / event-triggered campaigns tomorrow), suppression is enforced at TWO independent gates with a single shared classifier so both gates compute the same answer for the same member.
    - **Shared classifier** — `deriveSurveySuppression(member: { erased, unsubscribedSurveysAt, consentGivenAt, email })` in `packages/shared/src/distributionSuppression.ts` returns one of `'ok' | 'unsubscribed' | 'no-consent' | 'erased' | 'no-email'` and a sibling `since` ISO timestamp. The function is the single source of truth for both gates; the four input fields are *exactly* those that gate a survey send, and `Member.emailOptIn` is deliberately **NOT** part of the input shape — surveys are legitimate-interest exempt from the marketing-channel opt-out (spec OQ-NEW-1 / Round-7 reviewer decision). Co-located helpers `suppressionTooltip` + `suppressionChipLabel` keep operator-facing copy in lockstep.
    - **Gate 1 (UI / audience-builder)** — surfaces suppressed members in the audience list with a Status chip + disabled checkbox + explanatory tooltip so the operator's selection deterministically excludes them (R22 + R43). The annotation is computed server-side and surfaced on every audience-affecting endpoint: `GET /v1/members?q=…` (search results — `apps/api/src/routes/members.ts`) and `POST /v1/surveys/:id/distribution-batches/preview` (custom-list resolution — `apps/api/src/routes/distributionBatches.ts`). Frontend enforcement in `apps/web/src/app/(admin)/admin/surveys/[id]/distribute/_components/audience-builder/AudienceList.tsx` keeps suppressed checkboxes structurally disabled (`<input type="checkbox" disabled>` driven by `row.suppressionStatus !== 'ok'`) so the operator cannot select a suppressed row even with JS overrides.
    - **Gate 2 (worker pre-dispatch)** — re-checks suppression at the actual provider-call moment in `apps/worker/src/processors/managedEmailSend.ts:checkSuppression`, calling the same `deriveSurveySuppression` helper to catch the time-gap edge case (member unsubscribed in the seconds between Gate 1 selection and Gate 2 dispatch). A failed check transitions `SurveyDistribution` → Failed with a bounded `Skipped: <reason>` value (never retryable). The worker gate is structurally enforced — the helper's input shape physically prevents anyone from accidentally consulting `emailOptIn`.
    - **Reuse pattern**: for any future select-then-dispatch workflow (scheduled sends, event-triggered campaigns, batched outbound webhooks), both gates SHALL share one classifier. Adding a new suppression dimension means extending the classifier's input shape + return values once, and both gates pick it up automatically.

- **Email template rendering — inline-style + table-based layout** *(Issue #420; extended #540)*: HTML emails rendered server-side for cross-client compatibility (Gmail web/iOS, Outlook web/desktop, Apple Mail macOS/iOS) follow a constrained subset: inline `style="…"` only (no `<style>` blocks — Gmail web strips them); outer `<table>` for layout (Outlook desktop uses Word's rendering engine which struggles with flex/grid); divider via single-cell `<table>` row with `border-top` (raw `<hr>` styling is stripped by Outlook); `<a>` color set inline (Gmail iOS auto-recolors links without inline color). **`<img>` sizing requires both an HTML `width="…"` attribute AND inline `width:100%; height:auto; max-width:…px; max-height:…px;` style** — Outlook desktop's Word renderer honors the HTML attribute but ignores inline `max-*` (issue #540 production incident: large brand logos rendered at source size in Outlook). The HTML attribute sets the Outlook box; the inline `width:100%` lets modern clients use the CSS box up to `max-width`; `height:auto` preserves aspect ratio (resize, not crop). Operator-authored body HTML is rendered as-is but auto-styled: a small `rewriteLinksWithAccent` idempotent sweep applies the brand accent color to `<a>` tags without an explicit `style=` (operator-set `style=` wins). Mustache substitution (`{{first_name}}`, `{{survey_link}}`, etc.) happens via a simple regex (closed token set; no Handlebars dep). Theme palette resolution: `Survey.themeId` → `Brand.defaultThemeId` → shared `FALLBACK_RESPONDENT_THEME`. Validated against Chromium (Webkit-derived, proxy for Gmail web + Apple Mail macOS) in the #420 §9.3 spike; real-inbox cross-client check is a per-impl-PR pre-merge gate via developer-personal inboxes. Canonical implementation: `packages/shared/src/email/renderTemplate.ts`. Reuse for any future server-rendered email.

- **Backward-compatible POST endpoint extension via body discriminator** *(Issue #420)*: When a stable `POST /v1/<resource>/...` endpoint needs to grow a new behavior path without breaking existing callers, add a top-level discriminator field in the body (`sendMode` for #420; conceptually the same as `actionType` on existing campaign endpoints). Existing-shape requests (discriminator absent or set to the legacy value) MUST continue to behave identically — the new field defaults to the legacy semantic. Zod validation uses `z.discriminatedUnion(<field>, [...])` to enforce per-mode required fields without breaking the shared shape (e.g., `composer` required when `sendMode='MANAGED_EMAIL'`, `format` optional otherwise). Canonical implementation: `GenerateBatchRequestV2Schema` in `packages/shared/src/zod/distributionBatch.schema.ts`, consumed by `POST /v1/surveys/:id/distribution-batches`. Pairs with the **Hash-at-rest tokenized public endpoint** + **One-time secret regeneration** patterns (above) — composerSnapshot on the batch row freezes the theme-color palette at mint-time so the worker dispatch is deterministic even if `BrandTheme` changes mid-batch.

- **UI-resolved audience selections ship `memberIds[]`, not paste strings** *(Issue #531)*: When a client UI presents members from a server-authoritative search/preview and the operator selects from those results, the submission MUST carry the canonical `Member.id` for each selected row — not the row's `externalId` text. The receiving endpoint resolves UI-selected rows by `Member.id` (`prisma.member.findMany({ where: { id: { in: ids }, brandId, erased: false, deletedAt: null } })`) and bypasses any brand-identifier-kind-aware paste/CSV parser for those rows. Brand-kind-aware shape inference (`parsePasteBody` / `parseCsvBody`) is retained for genuinely typed operator input (raw paste, CSV upload, auto-enroll path). The two channels coexist on one audience-spec member as additive optional fields — `CustomListAudience.memberIds: z.array(z.string()).max(10_000).optional()` alongside the existing `identifiers: z.string()` paste body. The server merges both into one resolved-members set, deduped by memberId.
    - **Why** — the previous design round-tripped the search-result `externalId` through the paste parser. The parser's job is to validate that genuinely typed input matches `Brand.memberIdentifierKind` (EMAIL vs PHONE vs CUSTOMER_ID); applying that gate to rows the server itself returned from the same brand's member table caused a silent shape-disagreement drop and a non-recoverable `AUDIENCE_EMPTY` failure (production 2026-05-28, issue #531).
    - **Reuse rule** — any future UI surface that lets an operator pick from server-authoritative member-resolution results (CRM segments, A/B-test cohorts, batch operations) SHALL submit by `Member.id` for those rows. Shape-inference parsers serve typed input only.
    - **Canonical implementation** — `AudienceBuilder.tsx` splits `selectableRows` by `r.memberId !== null` and emits `{ memberIds, identifiers }` on the same `custom_list` audience. Server: `resolveCustomList(...)` in `apps/api/src/routes/distributionBatches.ts` resolves memberIds first, then the paste body.

- **Runtime-critical config — single in-repo source of truth + per-app declarative env vars, never silent placeholder** *(Issue #540)*: When server-side code reads a config value that appears in a user-visible artifact (URL embedded in a sent email, OAuth callback host, support-email domain, etc.), the resolver chain MUST be: env var with the per-deploy value → shared in-repo constant as the canonical fallback. NEVER a placeholder host like `app.customereq.example`, `https://localhost`, or an empty string. The shared constant lives at `packages/shared/src/constants.ts` (`PUBLIC_FRONTEND_HOST` + `PUBLIC_FRONTEND_URL` for the project's public origin) and is the loud, in-repo backstop the worker / API / shared code all import. A missing env var triggers a structured warn log (`event: 'frontend_url.fallback', reason: 'env_unset'`) so ops detects drift — but the user-facing artifact still ships a working URL, because the project knows its own canonical host.
    - **Paired requirement** — the CD pipeline sets the same env var declaratively on every deploy via `az containerapp update --set-env-vars` on **every consuming container app** (currently `customereq-api` AND `customereq-worker` — both run dispatch in `QUEUE_MODE=inline` and `QUEUE_MODE=redis` respectively; the API step is what unblocks current inline-mode prod). See `Set API non-secret env vars` and `Set Worker non-secret env vars` in `.github/workflows/deploy.yml`. Plain `--set-env-vars` is correct for non-secret public values; Key Vault references are required only for actual secrets per the `Production Secrets Policy` in `CLAUDE.md`.
    - **Resolver shape** — lazy named function (`resolveFrontendBaseUrl()` in `apps/worker/src/processors/managedEmailSend.ts`), not top-level `const`. Lazy evaluation lets unit tests manipulate `process.env` per-case without re-importing the module, and isolates the warn log to the actual dispatch path.
    - **Anti-patterns** — (1) `const x = process.env.X ?? 'https://placeholder.example'`. Placeholder hosts are also a passive security concern (a future attacker who registers the placeholder domain hijacks every dispatch where the env var drifted). Never default a user-visible URL to a host the project doesn't control. (2) Hand-written `'https://customereq.wellnessatwork.me'` literals scattered across files; they drift. Import the shared constant.
