...
- **Runtime-critical env config — loud-fail at first use, never silent placeholder** *(Issue #540)*: When server-side code reads an env var whose value appears in a user-visible artifact (URL embedded in a sent email, OAuth callback host, webhook signature key, etc.), the resolver MUST throw at first use if the variable is unset / empty — NOT fall through to a placeholder like `app.customereq.example`, `https://localhost`, or an empty string. Silent placeholder fallback was the failure mode that produced the #540 production incident: the `customereq-worker` Container App had no `NEXT_PUBLIC_FRONTEND_URL` set, the code fell through to the placeholder, and every survey email shipped with links pointing to a non-existent host. The throw guarantees the misconfiguration surfaces immediately (deploy fails, retries, or pages on-call) instead of degrading the user-facing artifact silently. **Paired requirement**: the CD pipeline MUST set the same env var declaratively on every deploy via `az containerapp update --set-env-vars` (see `Set Worker non-secret env vars` step in `.github/workflows/deploy.yml`) so the value is declarative + idempotent + survives container-app recreation. Plain `--set-env-vars` is correct for non-secret public values (sender domains, callback URLs); Key Vault references are required only for actual secrets per the `Production Secrets Policy` in `CLAUDE.md`.
    - **Resolver shape** — keep it a lazy named function that throws (`resolveFrontendBaseUrl()` in `apps/worker/src/processors/managedEmailSend.ts`), not a top-level `const` initialized at module load. Lazy evaluation lets unit tests manipulate `process.env` per-case without re-importing the module, and isolates the throw to the actual dispatch path (so a smoke test or migration script that imports the module but never calls dispatch doesn't crash on import).
    - **Anti-pattern** — `const x = process.env.X ?? process.env.Y ?? 'https://placeholder.example'`. Placeholder hosts are also a passive security concern (a future attacker who registers the placeholder domain hijacks every dispatch where the env var drifted). Never default a user-visible URL to a host the project doesn't control.
