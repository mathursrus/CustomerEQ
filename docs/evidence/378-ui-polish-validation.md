...
- 1 failure — `survey-lifecycle.test.ts` "activates a DRAFT survey via PATCH /v1/surveys/:id/status". **Pre-existing on `main`**: the `MISSING_TITLE` activation gate was added in commit 4311aef (`impl (#241) Slice 4b`, 4 days before this branch) but the `createSurvey` test-utils factory still defaults `title: null`. Per project Rule 21 (unrelated fix → separate issue + branch) — filed as [#414](https://github.com/mathursrus/CustomerEQ/issues/414) with a full diagnosis + verified scope (exactly one broken test in the entire codebase; all other PATCH-to-ACTIVE tests target different entities).
