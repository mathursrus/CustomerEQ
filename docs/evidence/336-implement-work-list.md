...
| 4 — implement-code | **Complete — items 1–12 of §J landed (2026-05-13)** | Walking skeleton + Basics/Consent/Questions/LookFeel/PointsAndThankYou tabs + Activate/Discard/ConsentAttestation modals all shipped. `/admin/surveys/new` replaced with thin Server Component (`auth()` → `GET /v1/programs` → `POST /v1/surveys` → `redirect`). Legacy deletions complete: `apps/web/src/app/(admin)/admin/survey-builder/page.tsx` + `apps/web/src/components/surveys/{TriggerStep,RuleBuilderStep,ReviewLaunchStep}.tsx` + `apps/web/src/utils/triggerRecommendation.{ts,test.ts}`. All-`apps/web` vitest: **253/253 green** across 29 files (was 79/79 after item 6; +30 = LookFeel 11 + PointsAndThankYou 11 + Activate 8 + Discard 5 = 35 tests added; net 30 once a few stub-driven flake-prone assertions were absorbed). `pnpm --filter @customerEQ/web typecheck` clean. `pnpm --filter @customerEQ/web lint` 0 errors (10 pre-existing warnings unchanged). `pnpm --filter @customerEQ/web build` succeeds — `/admin/surveys/new` route is 152 B (was a 462-line wizard); `/admin/surveys/[id]/edit` route is 10.8 kB; no `/admin/survey-builder` route in the output. Items 13 (architecture-doc MA1) → 14 (e2e finalize) belong to Phases 10 + 5 respectively per the ledger. |
| 5 — implement-validate | **Complete (2026-05-13)** | Full report at `docs/evidence/336-implement-validate.md`. All gates green: typecheck / lint / build / vitest 256/256 / Playwright e2e 17/17 / mobile-emulator (iPhone 12 profile) / API-shape diff / state-aware affordance audit. **Two real Phase 4 bugs surfaced and fixed in Phase 5**: (B1) ConsentAttestationModal wasn't wired in `SurveyEditorForm` — R10 violation; modal existed + unit-tested but never reachable; (B2) editor called `/v1/me` and `/v1/brand-themes` neither of which exist in `apps/api` — the actual endpoints are `/v1/admin/brand/profile` and `/v1/themes`; e2e mocks masked both. Also fixed 6 e2e spec bugs where Phase 3 specs assumed UI patterns Phase 4 didn't ship (row-menu trigger testid, flat chip filters not popovers, footer Discard button not More-menu, native select for consent). Added 3 RTL tests for the consent wiring + 1 mobile-emulator Playwright test (evidence at `docs/evidence/ui-polish/336/lookfeel-iphone12.png`). |
| 6 — implement-security-review | **Complete (2026-05-13)** | Full review section at `docs/evidence/336-feature-implementation-evidence.md` §Security Review. Diff-scoped OWASP Top 10 (web) + secrets scan + privacy/PII review against project R13. Single threat surface (`web`). One High finding (**A04-001**): ConsentAttestationModal sent `{ consentMode, consentReason, attestedBy }` but `UpdateConsentModeSchema.strict()` in `packages/shared/src/zod/survey.schema.ts:164` requires `{ consentMode, consentReason, attestation: { confirmed, reason } }`. R10 server enforcement remained intact (fail-closed) but the feature was unusable end-to-end; e2e mocks masked the contract mismatch. Inline-fixed per user authorization; commit `security(A04-001)`. Tests now lock the wire shape at unit + e2e layers. One Low SEC-LEAK accepted (test-fixture suppression). No blocking findings. |
| 7 — implement-regression | **Complete (2026-05-13)** | Unit 751/751 · Integration 380/380 · E2E full suite **142/207 pass after Tier-1 fixes**, **0 Slice 4b regressions remaining**, **17/17 Slice 4b specs green under 10-worker parallel load** (`pnpm --filter @customerEQ/web exec playwright test 336-*.spec.ts --workers=10`). Tier-1 fixes landed in this session: (a) deleted 3 orphaned legacy specs (Phase 4 §C.5 cleanup oversight: `survey-creation.spec.ts`, `survey-rule-builder.spec.ts`, `survey-trigger-wizard.spec.ts`), (b) `335-survey-detail-page.spec.ts` updated for the 4-section R32b detail page + `'Share link'` strict-mode collision with the Distribution DRAFT-state banner fixed (`exact: false` → `exact: true`), (c) `admin-nav-scrollable.spec.ts:66` typo `toBeAttachedToDOM()` → `toBeAttached()`, (d) parallel-worker hardening on 3 Slice 4b tests (`336-surveys-list:167` row-click, `336-survey-editor:275` Activate, `336-survey-editor:299` Discard) — bumped post-mutation `toHaveURL` timeouts from 10 s → 30 s + added `waitForLoadState('networkidle')` before the list row click. 30 pre-existing failures filed as GitHub issues #358 (Member Enrollment), #359 (Member Portal Rewards), #360 (Loyalty Programs), #361 (External Signals + Integrations), #362 (MCP OAuth) + 4 already covered by existing #312 (Themes); 1 `campaign-edit.spec.ts:67` documented as parallel-flake (passes serial, pre-existing per Slice 4a) — all 31 grouped by surface area per user direction so other contributors can pick them up. Full Phase 7 detail in `docs/evidence/336-feature-implementation-evidence.md` §Regression Triage. |
| 8 — implement-quality | **Complete (2026-05-13)** | Full feedback ledger at `docs/evidence/336-feature-implementation-feedback.md`. 5 findings (Q8-001 through Q8-005), all ADDRESSED. Key refactors: (a) extracted `parseErrorResponse()` to `apps/web/src/lib/errors.ts` and refactored 4 modal call sites to remove 28 lines of duplicate error-parsing (Rule 15); (b) extracted `<ModalShell>` to `apps/web/src/components/ModalShell.tsx` and migrated 4 modal consumers — backdrop standardized on `bg-gray-900/50` (was 3 different shades across 5 modals — `bg-gray-900/50`, `bg-black/40`, `bg-slate-900/55`); (c) fixed dead `url.startsWith('http') ? url : url` ternary in `page.tsx`; (d) named the magic 60_000 / 60 constants in `TabHeader.tsx`; (e) bonus: removed pre-existing unused `MOCK_THEME_DEFAULT` import in `page.test.tsx` to restore `pnpm lint` to 0 errors. Verification: `pnpm --filter @customerEQ/web typecheck` clean · `pnpm --filter @customerEQ/web lint` 0 errors (warnings unchanged) · `pnpm --filter @customerEQ/web test` **256/256 passed** · `pnpm --filter @customerEQ/web exec playwright test 336-*.spec.ts --workers=10` **17/17 passed in 1m12s**. Note: `ImpliedAttestationModal.tsx` in `/admin/settings/organization/` uses the same modal pattern and could migrate to `<ModalShell>` — flagged for #292 follow-up, out of scope here per R21. |
| 9 — implement-completeness-review | **Complete (2026-05-13)** | Feature Requirement Traceability Matrix (25 rows: 15 #336 ACs + 10 #241 spec rules R1/R3/R5/R6/R10/R11/R12/R13/R14/R17/R18/R19/R20/R21/R23/R30/R32b) — **25/25 in-scope Met**; 2 phase-deferred rows (AC13 → Phase 10 MA1, AC15 → Phase 11 CI). Technical Design Traceability Matrix (17 rows from RFC §"File tree", §"State-aware save behavior", §"RHF form structure", §"Question canvas", §"Save behavior by state", §"BrandTheme → Survey element token mapping" + 2 Phase-8 extracted-primitives entries) — **17/17 Met**. Feedback verification: 5 Q8 findings + 1 bonus all ADDRESSED, 0 UNADDRESSED. Validation-mode audit clean (mobile emulator + API-shape diff + state-aware affordance audit all performed). Full report at `docs/evidence/336-feature-implementation-evidence.md` §Completeness Review. |
| 10 — implement-architecture-update | **Complete (2026-05-13)** | MA1 landed in two places per work-list §C.6 row 34: (a) `docs/architecture/architecture.md` §6 — new bullet "State-aware save mode for admin editor forms" with full DRAFT/ACTIVE/PAUSED/STOPPED behavior, useAutoSave hook reference, pairing rule with the server-side state-aware PATCH editability allowlist (Slice 2); (b) §3.1 — new bullet under Presentation Layer "State-aware save trigger on the edit route" pointing to the Slice 4b implementation as the reference. Last of the umbrella #324's three MA commitments now landed. |
| 11 — implement-submission | **Complete (2026-05-13)** | Branch pushed (11 commits Phases 1–10) and PR opened — [#364](https://github.com/mathursrus/CustomerEQ/pull/364) "impl (#241) Slice 4b: full editor (4 tabs) + /new Server Component + legacy survey-builder cleanup". PR body includes summary · what lands operationally · phase ledger · verification gates · references · test plan. PR labels: enhancement / surveys / phase:implement / tech-debt. Evidence comment posted on PR. Issue #336 labels updated: added `status:needs-review`. |
| 12 — address-feedback | **In progress — hold-point active** | Round 1 (33 V1-* manual-verification items + 4 Q12-* quality findings) and Round 2 (1 V2-* CI lint fix) all ADDRESSED. Ledger at `docs/evidence/336-feature-implementation-feedback.md`. All sub-phase re-validations Pass. Latest pushed commit: `e33ac16` (V2-001 dead-symbol removal). Awaiting CI green on run `25872596587` and explicit user signal before completing the phase. Never auto-complete (Rule 25a). |
...
**Next session entry point**: `fraim_connect` (a fresh session ID is issued each connect — do not try to reuse the old one; FRAIM session state is reconstructed from the work-list + this resumption pointer, not from the session ID), then call `seekMentoring({ jobName: 'feature-implementation', jobId: '336', issueNumber: '336', currentPhase: '<next-phase>', status: 'starting' })` where `<next-phase>` is the next pending row in §M's phase ledger. This work-list is the authoritative scoping artifact — read it before §M to know what landed, then call `seekMentoring`. **Phases 1–11 are complete.** PR is open at [#364](https://github.com/mathursrus/CustomerEQ/pull/364). Next phase is **Phase 12 — address-feedback** — this is a FRAIM **hold-point** (per project Rule 25a) that waits for an explicit user signal that PR review feedback has landed before resuming. Do NOT auto-advance; do NOT call `seekMentoring(status='complete')` on Phase 12 until the user explicitly says "proceed" / "feedback addressed" / "check the PR" / "PR merged" / "approved".

**Session 2026-05-14 (Phase 12 Round 1 + Round 2) — resumption checklist for the next session**:

Phase 12 is **in progress** — two rounds of feedback have been ADDRESSED and re-validated. The hold-point is still active.

1. **Check CI on the latest commit first.** Latest pushed commit: `e33ac16` ("fix(#336): V2-001 — remove dead unused symbols failing CI lint"). When this session ended, CI run `25872596587` was in progress on this commit. Confirm green before treating Round 2 as fully verified:
   ```
   gh run list --branch feature/336-impl-241-slice-4b-full-editor-4-tabs-new-server-component-legacy-survey-builder-cleanup --limit 4
   ```
   The two previously-failing checks (push + pull_request, both named "Build, Lint, Test") were failing on `pnpm build` lint errors for `TabHeader.tsx:49 buildIndicator` and `(admin)/layout.tsx:5 useEffect`. Both removed in `e33ac16`.

2. **Read both feedback rounds.** `docs/evidence/336-feature-implementation-feedback.md` contains:
   - Round 1: 33 V1-* items (V1-001..V1-033) + 4 Q12-* quality findings — all ADDRESSED. Round 1 commits: `312ce64` (fix), `52549b7` (ledger), `b3d4b19` + `5dbcb0c` (test alignment), `ce276ee` (Q12-001 RendererErrorLine extraction), `8a70082` (Phase 12 implement-architecture-update doc).
   - Round 2: 1 V2-* item (V2-001 dead-symbol removal) — ADDRESSED in commit `e33ac16`. The Round 2 item also produced a coaching moment about Phase 12 implement-validate needing to run `pnpm build`, not just typecheck — see file at `fraim/personalized-employee/learnings/raw/manohar.madhira@outlook.com-2026-05-14T16-40-00-validate-must-include-build.md`.
   - The PR comment summarizing Round 1 re-submission lives at https://github.com/mathursrus/CustomerEQ/pull/364#issuecomment-4452633818. Round 2's status is not yet posted to the PR — wait until CI is green before adding a Round 2 follow-up comment.

3. **Decide based on user input.** When the user resumes:
   - If they say "proceed" / "approved" / "merged" / "CI is green, advance" → call `seekMentoring({ ..., currentPhase: 'address-feedback', status: 'complete', findings: { approved: true, feedbackRounds: 2 } })` to advance to **Phase 13 — retrospective**.
   - If they report new feedback → start **Round 3** in the feedback ledger using the same V3-NNN slug convention, address items, re-run the relevant Phase 12 sub-phases (`implement-validate` MUST include `pnpm build` this time, see coaching note in §2 above), commit + push, then call `seekMentoring({ ..., currentPhase: 'address-feedback', status: 'failure', findings: { roundNumber: 3, ... } })` to re-loop.
   - If CI is still failing on `e33ac16` → diagnose with `gh run view <run-id> --log-failed` and fix on the same branch (Round 2 stays open until CI is green).

4. **Other state on disk at session end**:
   - Branch head: `e33ac16` (pushed to origin).
   - Working tree: clean as of the last `git status -s` call.
   - Local dev servers (api / web / docker): the user said earlier "you can close my dev servers if you need to." If anything is left running it can be killed; no in-flight test runs depend on them.
   - The 5 raw coaching learnings from Round 1 (`always-open-html-mocks`, `silent-scope-removal`, `fraim-phase11-stay-on-pr`, `copy-env-from-main-worktree`, `mock-drift-is-my-responsibility`) + the 1 from Round 2 (`validate-must-include-build`) are all under `fraim/personalized-employee/learnings/raw/` waiting for the next `sleep-on-learnings` synthesis pass.

**Note on Phase 4 progress (after session 2026-05-13c)**: All four remaining tabs/modals are now implemented. Per-question right-rail in `QuestionsTab` covers **text · type · required**; skip-logic and branching UIs are **deferred** to a follow-up issue — no Slice 4b AC enforces them, no test exercises them, and per R21 they belong on their own branch rather than getting bundled here. `LookFeelTab` chrome-matrix toggles propagate the **full** matrix (both channels) on each change rather than a partial diff — keeps the autosave PATCH body self-contained. `PointsAndThankYouTab` reserves the V1 points-override slot as an empty `<div data-testid="points-override-slot" />` per §E hide-vs-stub (slot present in DOM; no copy). The `Discard draft` CTA is rendered as a footer button only when `status === 'DRAFT'`; a future More-menu refactor can hoist it into the editor header (out of scope for #336).
