...
| 3 (self-audit) | 2026-05-19 | 6 self-found items (5 line-number / consistency drifts + 1 fabricated `#262` provenance claim on the 50k export cap) | All marked ADDRESSED. Audited every concrete file-path / line-number / schema / constant / URL / cross-issue-pattern in the spec body against actual repo state. Caught: `SurveyResponse` line was 759 in spec / 805 in schema (drift); `Brand.timezone` lines were 212–213 / 213–214 (off by 1); `ResponseSection.tsx` placeholder span 11–13 / 10–13 (under-claim); `surveys.ts` detail-endpoint `deletedAt: null` 124 / 125 (comment vs clause); filter-row order in §4 edge case still listed Round-1 order; **50k cap's "matches #262 SurveyImportBatch ceiling" was fabricated — no such cap exists in repo. 50k value retained but justified standalone.** Spec is now fully verified against current repo state. |
