...
| Issue Problem 2 — "`?member_id=` in the URL is a security and privacy liability" | NFR-S4 (no PII in URL); R13 (token plaintext shown once); R16 (URL shape `/survey/:surveyId/r/:token` — R3.1-corrected path extending the existing `/survey/<id>` share-link path per code at `apps/web/src/app/(admin)/admin/surveys/[id]/components/DistributionSection.tsx:109`); R17 (token-resolves-member supersedes body identifier); Compliance §GDPR Art. 5(1)(c); Alternatives table (5 rejected URL-identifier approaches, including bearer-token-in-query-string which leaks identically to PII-in-query-string) |
