...
- [ ] NEW `apps/api/src/auth/clerk-identity-provider.ts` — Clerk implementation wrapping `@clerk/clerk-sdk-node` and `svix`. Honors the `createUserWithOrg` cleanup contract. Per RFC §3.2. **Round 1 update (PR #197)**: constructor takes a `logger` parameter; orphan-cleanup paths use `this.logger.error(...)` instead of `console.error`. `signInUser` method **removed** from interface and impl per reviewer choice (option a) — sign-in is browser-driven via Clerk.js after `createUserWithOrg`.
