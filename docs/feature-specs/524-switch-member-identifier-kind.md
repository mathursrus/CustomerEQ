...
### Brand-side cutover
- **R30** — Before the admin confirms the migration, the flow SHALL present a **data-driven impact preview**. It SHALL list **only the integration surfaces that send the brand external identifier and have non-zero activity in the last 30 days** — embedded-form survey responses, manual API enrollments, Custom List distribution batches, share-link survey responses, plus active outbound webhook subscriptions as an informational heads-up — ordered by **most-recent-activity first**, each row showing a last-seen-at timestamp and the brand-side action required. Surfaces that key on the internal member id (e.g. `POST /v1/events`, which uses the stable internal `Member.id`) are migration-stable and SHALL NOT be listed. Surfaces with zero activity in the window SHALL be omitted.
...
- **R32** — During the grace window, inbound member resolution SHALL accept the old (`customer_id`) or the new (`email`) identifier on the external-identifier ingress paths (the `resolveOrEnrollMember`-backed set in R19: public survey respond, manual API enroll, distribution-batch audience), extending R19's dual-key behavior past the kind flip until the grace deadline.
- **R33** — During the grace window, the system SHALL record old-key resolution events per ingress source (public survey respond, manual API enroll, distribution-batch audience) with brand-scoped counts.
...
- **R35** — After the grace deadline passes, inbound requests supplying the old `customer_id` SHALL NOT resolve to a member and SHALL NOT create a new member. If the old `customer_id` matches a retained migration mapping, the request SHALL be rejected `410 IDENTIFIER_DEPRECATED_AFTER_MIGRATION` naming the member's new identifier shape; otherwise it SHALL fall through to ordinary identifier-shape validation for the current kind (`422 IDENTIFIER_SHAPE_INVALID`) naming the brand's current kind. Both rejections SHALL be observable in the brand's usage logs. (The migration mapping is retained post-grace to enable the actionable error — see R26.)
...
- **R26** — The re-key SHALL preserve `consentGivenAt`, `consentVersion`, erasure flags (`erased`, `deletedAt`), `clerkUserId`, point balances, and all member-linked history (which joins on the stable `Member.id`, not `externalId`). The migration mapping rows SHALL be retained after grace expiry to support R35's actionable error.
...
- **R36** — The migration SHALL operate only on **loyalty members** (`Member` rows, keyed by `externalId`). Admin / portal users — Clerk organization members provisioned as a `Brand` (not `Member` rows; `user.created` is a no-op in the identity webhook) — SHALL NOT be affected. A loyalty member that additionally carries a `clerkUserId` (self-enrolled login) SHALL be migrated like any loyalty member, with `clerkUserId` preserved per R26.
...
- **R31/R32** — *Given* a migration whose kind flip completed 5 days ago, *when* a backend re-enrolls an existing customer via `POST /v1/members/enroll` with `memberId: cust_00012` (the old identifier), *then* the request resolves to the migrated member and is not rejected.
- **R33/R34** — *Given* a brand 10 days into the grace window, *when* the admin views the Member identification section, *then* it shows the grace deadline, the per-ingress old-key usage (e.g., "247 enrollments via `/v1/members/enroll`, 0 via embedded forms"), and an "Extend grace window" action.
- **R35(a)** — *Given* a grace window that expired yesterday, *when* a backend sends the old `cust_00012` (a migrated member) to `POST /v1/members/enroll`, *then* it is rejected `410 IDENTIFIER_DEPRECATED_AFTER_MIGRATION` with the new email shape in guidance, and no member is created.
- **R35(b)** — *Given* the same expired window, *when* a backend sends a brand-new `cust_NEW` (never migrated) to `POST /v1/members/enroll`, *then* it is rejected `422 IDENTIFIER_SHAPE_INVALID` naming the brand's current kind (EMAIL), and no member is created.
- **R36** — *Given* a brand with 1,284 loyalty members and 3 admin/portal Clerk users, *when* the migration runs, *then* coverage totals show 1,284 (not 1,287) and the admin users' Clerk logins are unaffected; *and given* one loyalty member has a `clerkUserId`, *then* after re-key that member's `clerkUserId` is unchanged.
...
- **R37** — *Given* a grace window with 5 days remaining and 1,847 old-key enrollments still hitting `/v1/members/enroll` in the last 7 days, *when* the admin loads any admin page, *then* a brand-wide warning banner displays "5 days left — manual API enroll still using Customer ID" with a one-click link to the grace-status panel.
