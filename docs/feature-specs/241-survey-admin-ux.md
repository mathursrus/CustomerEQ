...
- **Thank-you message** — textarea with insert-variable picker. Available variables: `{{points}}`, `{{programName}}`, `{{pointCurrencyName}}`, `{{rewardLink}}`, `{{memberName}}`. `{{points}}` resolves at render time to the points credited for the specific response (program base rate × any active campaign multipliers). Default copy: "Thank you for your feedback! Your `{{points}}` `{{programName}}` `{{pointCurrencyName}}` are on their way to your account."
...
🤖 Spec converged after 5 review rounds (R0 seed + 4 user feedback rounds + 1 architectural reversal). Implementation RFC and validation matrix follow this PR.
