...
The behaviors above expressed as testable SHALL statements with `Given / When / Then` acceptance criteria. Tags `R1`–`R22` are referenced from implementation tasks, tests, and the RFC's traceability matrix.
...
| **R8** | The Response policy field SHALL be a single-select dropdown with three options: Allow multiple responses (default) / One response per member / Latest overwrites prior. | Given a brand has surveys created before this Epic, when the migration runs, then every existing `Survey.responsePolicy` is `MULTIPLE` (matches #231 R14). |
