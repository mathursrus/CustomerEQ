...
Status: **Ready for review (round 8 — RCA preventive controls applied retrospectively per CTRL-5; awaiting reviewer signoff).**
...
|----|-------------|---------------------------------------------|
| **R1** | The survey list page SHALL provide one primary `+ New survey` CTA. Existing entry points (`/admin/surveys/new` adhoc form, `/admin/surveys/new` triggered wizard, `/admin/survey-builder`) SHALL redirect to `/admin/surveys/[id]/edit` for the appropriate survey row. | Given the admin lands on `/admin/surveys`, when they click `+ New survey`, then a new `Survey` row is created with `status = DRAFT` and the browser navigates to `/admin/surveys/[id]/edit` on the Basics tab. |
...
| ID | Requirement | Acceptance Criterion (Given / When / Then) |
|----|-------------|---------------------------------------------|
| **R6** | The Survey type field SHALL be a 4-card grid: NPS / CSAT / CES / Custom. Selecting NPS / CSAT / CES SHALL auto-populate the Questions tab with that type's standard preset set (all editable, all reorderable, all deletable). Custom SHALL leave the Questions canvas blank. | Given the admin picks NPS, when they switch to the Questions tab, then the canvas shows the NPS preset banner and the standard NPS question set. Given the admin picks Custom, then the canvas shows zero questions and the palette is the only source of new questions. |
...
| **R8** | The Response policy field SHALL be a single-select dropdown with three options: Allow multiple responses (default) / One response per member / Latest overwrites prior. **Anonymous responses** (`SurveyResponse.memberId IS NULL`) SHALL bypass response-policy enforcement entirely — every anonymous submission is stored as a new row regardless of the configured policy, because there is no member identity to enforce uniqueness against. | Given a brand has surveys created before this Epic, when the migration runs, then every existing `Survey.responsePolicy` is `MULTIPLE` (matches #231 R14). Given `responsePolicy='ONCE'` and an anonymous respondent submits twice, when both submissions complete, then two distinct SurveyResponse rows exist (both with `memberId=NULL`). |
...
| **R16** | Embedded-distributed surveys (widget JS mounted on host page) SHALL consume member identity from the URL query parameter (`?member_id=…`) at mount time in V0. If the parameter is missing, the embedded form SHALL fall back to the same identification prompt as Standalone (R15). | Given the host page mounts the widget with `?member_id=mbr_abc`, when the form renders, then no identification field is shown and the response is associated with `mbr_abc`. Given the host page mounts the widget without `member_id`, when the form renders, then the identification field is shown per R15. |
...
| **R28** | The Configuration summary section SHALL render the actual configured survey form as a first-class element (reusing the Look & Feel `preview-survey` component) on the left, with a compact text summary on the right. | Given the admin expands Configuration summary, when the section body renders, then a rendered preview of the survey form appears alongside dl-style text summary entries for basics / look & feel / points / consent. |

### Open Questions (RFC / implementation phase)

These items are unresolved at spec-converge time and resolve during the RFC / implementation:

- **OQ1** — When a program has no `EarningRule(triggerEvent='survey_completion')` and a member submits a response, the worker silently skips earning (no `LoyaltyEvent` row, no error). Is this the intended behavior, or should the worker emit a structured warning to the admin dashboard? *Recommendation*: silent skip in V0; the editor's "No points configured" indicator handles the operator-facing visibility.
...
🤖 Iteration history: R0–R8. The R7 audit applied the R6 RCA's controls forward-only and missed retrospective application; R8 adds the missing controls (CTRL-5 retrospective sweep, CTRL-6 Epic-AC traceability, CTRL-7 mock-spec parity sweep, CTRL-2 amendment on convergence vocabulary) and applies them across the existing spec. **Convergence is the reviewer's call** — this spec awaits reviewer signoff on PR #314 before being declared converged.
