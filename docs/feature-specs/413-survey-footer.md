...
**Anchor target** (see OD-7 for domain selection):

```
https://customereq.com/?utm_source=survey_footer&utm_medium={channel}&utm_campaign=powered_by
```
...
- **Link text**: visible text reads `Powered by CustomerEQ` with "CustomerEQ" as the anchor. Screen-reader text on the anchor: `aria-label="Powered by CustomerEQ — opens customereq.com in a new tab"` so the new-tab behavior is announced.
...
| **R8** | The footer anchor SHALL be keyboard-focusable, SHALL render a visible focus indicator per RFC #241 §"Focus-visible outline" (2px outline at `--ceq-primary-color`, 2px offset on themed surfaces; `#6366f1` on non-themed), and SHALL carry `aria-label="Powered by CustomerEQ — opens customereq.com in a new tab"`. | Vitest + RTL: render `SurveyFormRenderer`; query the footer link; assert `tabIndex` reachable (no `tabIndex=-1`), `aria-label` matches the expected string, `target="_blank"`, `rel="noopener noreferrer"`. Focus-state asserted via a `:focus-visible` style snapshot. |
...
| **OD-7** | **Marketing-site domain** — which TLD does the footer link to? | **`https://customereq.com/`** ← recommended | (a) `https://customereq.io/`; (b) leave the host configurable via env (`MARKETING_SITE_URL`) | The repo uses **both** `customereq.com` (support email at `support@customereq.com`, CDN at `cdn.customereq.com`) and `customereq.io` (API at `api.customerEQ.io`, embed CDN at `cdn.customereq.io/embed/`). The `.com` is the user-facing brand surface — support contact + components CDN — while `.io` is reserved for technical subdomains. Marketing belongs on the user-facing surface. (b) is over-engineered for a value that effectively never changes after launch; per *Merit over ease*, env-configuration is the shortcut for indecision, not the durable answer. **Reviewer to confirm — if neither `customereq.com` nor `customereq.io` is intended as the marketing root, this needs to be resolved before implementation.** |
...
| **WCAG 2.1 AA** | [3.2.5 Change on Request](https://www.w3.org/WAI/WCAG21/quickref/#change-on-request) | `target="_blank"` is announced via `aria-label="Powered by CustomerEQ — opens customereq.com in a new tab"` so the new-tab navigation is not surprising. |
...
| **Manual (screen reader)** | NVDA / VoiceOver announces the footer correctly | Open the survey in NVDA (Windows) / VoiceOver (macOS); listen for "Powered by CustomerEQ — opens customereq.com in a new tab, link" |
