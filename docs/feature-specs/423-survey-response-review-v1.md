...
2. **Filter row** — chip-style, lifted to **shared filter modules** so every future surface (cluster detail, member detail, CX Insights, etc.) consumes the same primitives instead of forking:
   ...
   - **Score band** — chip options derived from `Survey.type` at render time. Constants live in `packages/shared/src/constants.ts` (extends the existing `NPS` constant with sibling `CSAT` and `CES`).
     - NPS → `Promoter (9–10) · Passive (7–8) · Detractor (0–6)`
     - CSAT (1–5) → `Satisfied (4–5) · Neutral (3) · Dissatisfied (1–2)`
     - CES (1–7) → `Easy (5–7) · Neutral (4) · Hard (1–3)` (modern CES 2.0 — *"the company made it easy"*, high = good)
     - Group label is *"Score band"*, never *"Type"* — the survey type is fixed.
   - **Sentiment band** — chip options `Positive · Neutral · Negative`, derived from the stored `SurveyResponse.sentiment` float using thresholds in `packages/shared/src/constants.ts` (`SENTIMENT.POSITIVE_THRESHOLD = +0.33`, `SENTIMENT.NEGATIVE_THRESHOLD = -0.33`). The group is hidden when the survey has zero open-ended text questions (sentiment is never populated for those). A small info icon next to the group label surfaces the multi-text-question caveat (see column section below).
   - **Channel** — multi-select chips over the distinct `channel` values present (`email · in_app · link · sms · review`).
   - **Filter row stays one row.** Default order: `Submitted · Score band · Sentiment band · Channel`. If the wrap-aware row would exceed one row at default page width, **Channel** collapses behind a `More filters ↓` popover with the channel chips inside. Channel is the overflow candidate (everything above it is more analytically useful for triage).
   - Filters intersect across groups, union within a group. Defaults: `All time`, all Score Bands selected, all Sentiment Bands selected, all Channels selected.
3. **Table**, one row per response. Columns (left to right):
   ...
   - **Sentiment** *(AI-derived, today)* — a small colored chip from the stored `SurveyResponse.sentiment` float bucketed via the `SENTIMENT` constant: `Positive` (green), `Neutral` (slate), `Negative` (red). Raw float visible on hover (`+0.62`, two decimals). Empty when the response has no open-ended text.
   - **Topics** *(AI-derived, today)* — up to 3 chips from `SurveyResponse.topics` (alphabetised). `+N` overflow chip when there are more; hover reveals the full list. Empty when `topics` is an empty array.
   - **AI summary** *(AI-derived, today)* — `SurveyResponse.summary` (one short prose line). Truncate + click-to-expand-in-row like other free-text cells. Empty when null.
   ...
   - **AI-column caveat indicator.** A small info-icon sits at the right edge of the Sentiment / Topics / AI summary column-group header, surfacing on hover: *"AI-derived columns are computed across all open-ended answers on each response. For standard NPS / CSAT / CES surveys with one open-ended question, the values are correct. For multi-text-question surveys, per-question synthesis lands in a later phase of #235 — interpret with that caveat. See [#235](https://github.com/mathursrus/CustomerEQ/issues/235)."* The caveat is also written into the export cover block.
...
1. **Land on the survey detail page.** The Survey Owner clicks a survey in `/admin/surveys`, lands on `/admin/surveys/[id]`. The Response section is auto-expanded (responses exist). They see the table loaded with the most recent 25 rows — Member, Channel, Submitted, Score (tinted by band), **Sentiment chip, Topics chips, AI summary**, then per-question columns. Default sort by `Submitted` descending.
2. **Filter to detractors.** They click `Detractor` in the **Score band** chip group. Count badge updates from `412 responses` to `89 responses`. Score-cell tints in the table are all red now.
3. **Narrow further to Negative sentiment.** They click `Negative` in the **Sentiment band** group. Count drops to `54`. The Sentiment chips visible in the column are all red.
4. **Filter to a wave.** They click the wave dropdown above the section (the #378 selector). Choose `Q2 2026 NPS · 2026-04-15 · 87 / 100`. Count drops to `31` — the detractors with negative sentiment inside the Gold-tier wave.
...
6. **Expand a long verbatim.** A detractor row's open-ended answer is truncated to *"The reps were polite but I waited 40 minutes for someone to acknowledge…"* The operator clicks `more`. The row expands in place, the full text renders alongside the AI summary one-liner ("Customer frustrated with support response time despite polite agents"). They click `less`; the row collapses.
7. **Read the AI-column caveat.** Hovering the info-icon at the end of the AI-column group reveals the multi-text-question caveat. This survey has one open-ended question, so the operator trusts the sentiment chip directly.
8. **Export to Excel.** They click `Export to Excel`. A file `survey-q2-2026-customer-pulse-responses-2026-05-19.xlsx` downloads. The first 8 cover-block rows include the active Score Band and Sentiment Band filters alongside Wave, Date range, Channels, the AI-fields caveat line, and total row count. The data sheet starts after the cover with full question headers, AI-derived columns (Sentiment, Topics, AI summary), and full verbatim answers.
...
- **R2.** Each row SHALL show: Member, Channel, Submitted, Score, **Sentiment** (AI-derived chip from `SurveyResponse.sentiment`), **Topics** (AI-derived chip list from `SurveyResponse.topics`), **AI summary** (one-line text from `SurveyResponse.summary`), and **one column per question** in `Survey.questions` order, the cell containing the verbatim value from `SurveyResponse.answers[questionId]`.
...
- **R6a.** The AI-derived columns (Sentiment chip, Topics chips, AI summary) SHALL render the stored `SurveyResponse.sentiment / topics / summary` values directly — no re-analysis on display. An inline info-icon at the AI column group surfaces the multi-text-question caveat described in §2.3.
...
- **R9a.** A **Score Band** chip group SHALL filter the table server-side. Chip options SHALL be derived from `Survey.type` at render time: NPS → `Promoter / Passive / Detractor`; CSAT → `Satisfied / Neutral / Dissatisfied`; CES → `Easy / Neutral / Hard`. Band-to-score boundaries SHALL come from `packages/shared/src/constants.ts` (`NPS`, `CSAT`, `CES` — added in this issue's impl phase, paralleling the existing `NPS` constant).
- **R9b.** A **Sentiment Band** chip group SHALL filter the table server-side. Chips: `Positive (sentiment ≥ SENTIMENT.POSITIVE_THRESHOLD)`, `Neutral (between thresholds)`, `Negative (sentiment ≤ SENTIMENT.NEGATIVE_THRESHOLD)`. Boundaries SHALL come from `packages/shared/src/constants.ts.SENTIMENT` (`POSITIVE_THRESHOLD = +0.33`, `NEGATIVE_THRESHOLD = -0.33`). The chip group SHALL be hidden when the survey has zero open-ended text questions.
...
- **R9d.** The filter bar SHALL render its chip groups on a single row at default detail-page widths. If the wrap-aware row would exceed one row, **Channel** SHALL collapse behind a `More filters ↓` popover. Channel chip selection state SHALL be preserved across collapse/expand.
- **R10.** Wave + Submitted + Score Band + Sentiment Band + Channel filters SHALL compose by intersection — a response appears iff it matches every active group.
...
- **R15.** The exported workbook SHALL begin with a cover block (rows 1–9) containing, one labeled key-value pair per row in order: **Survey** (`name · type · id`), **Exported at** (timestamp in brand TZ + locale), **Exported by** (operator email), **Wave** (active wave selection label — `All waves and direct responses`, the batch label, or `Direct responses`), **Submitted range** (date range as displayed, brand TZ), **Score band** (comma-separated active bands or `All bands`), **Sentiment band** (comma-separated active bands or `All sentiments`; `N/A` when survey has no open-ended text questions), **Channels** (comma-separated active channels, or `All channels` when none deselected), **Total rows** (the filtered total). Row 10 is intentionally blank; row 11 is a single italicised disclaimer cell *"AI-derived columns (Sentiment / Topics / AI summary) are computed across all open-ended answers per response. For multi-text-question surveys, per-question synthesis lands in a later phase of #235."* Row 12 is blank; data starts at row 13.
- **R16.** Exported column headers SHALL contain the **full** question text (no truncation). The exported sheet SHALL include the AI-derived columns (Sentiment, Topics, AI summary) in addition to the per-question columns. Free-text answers SHALL be preserved verbatim (no truncation, no AI re-synthesis applied at export time).
...
- **R18a.** The Export endpoint SHALL refuse to render an `.xlsx` for a filter set whose `total > 50000` and return HTTP 413 with body `{ code: 'EXPORT_TOO_LARGE', total, capacity: 50000, message: '<filter help>' }`. The UI SHALL pre-emptively disable the Export button when the filtered total exceeds 50,000 (the count badge already knows it), avoiding the wasted round-trip.
...
- **Score-band derivation:** for each `Survey.type ∈ {NPS, CSAT, CES}`, `bandOf(score)` SHALL return the expected band per the constants in `packages/shared/src/constants.ts`. Boundary cases: NPS `6 → Detractor, 7 → Passive, 8 → Passive, 9 → Promoter`; CSAT `2 → Dissatisfied, 3 → Neutral, 4 → Satisfied`; CES `3 → Hard, 4 → Neutral, 5 → Easy`.
...
- XLSX shape: parse the `.xlsx` body in the test, assert (a) cover-block rows 1–9 contain the named filter labels; (b) row 11 contains the AI-fields disclaimer; (c) data header row contains the three AI-derived column headers in addition to per-question headers.
