...
2. **Filter row** — chip-style, lifted to **shared filter modules** so every future surface (cluster detail, member detail, CX Insights, etc.) consumes the same primitives instead of forking. **Default order:** `Score band · Sentiment band · Submitted · Channel`.
   ...
   - **Sentiment band** — chip options `Positive · Neutral · Negative`, derived from the stored `SurveyResponse.sentiment` float using thresholds in `packages/shared/src/constants.ts` (`SENTIMENT.POSITIVE_THRESHOLD = +0.3`, `SENTIMENT.NEGATIVE_THRESHOLD = -0.3`). *(RFC §10 OQ-3 corrigendum — earlier drafts of this spec listed ±0.33; the canonical constant has always been ±0.3 with strict `<` / `>` semantics. Five unrelated surfaces (`apps/worker/src/processors/sentimentAnalysis.ts`, `apps/api/src/routes/analytics.ts`, `apps/web/src/app/(admin)/admin/analytics/cx/...`, `apps/web/src/app/(admin)/admin/members/[id]/page.tsx`) consume the same constant; changing it would reclassify rows on those surfaces with no operator benefit at the ±0.03 band-width.)*
...
- **R9b.** A **Sentiment Band** chip group SHALL filter the table server-side. Chips: `Positive (sentiment > SENTIMENT.POSITIVE_THRESHOLD)`, `Neutral (between thresholds, inclusive)`, `Negative (sentiment < SENTIMENT.NEGATIVE_THRESHOLD)`. Boundaries SHALL come from `packages/shared/src/constants.ts.SENTIMENT` (`POSITIVE_THRESHOLD = +0.3`, `NEGATIVE_THRESHOLD = -0.3` — corrigendum per RFC §10 OQ-3). The chip group SHALL be hidden when **either** (a) the survey has zero open-ended text questions, **or** (b) `Survey.type ∉ {NPS, CSAT, CES}` — band-style filtering is reserved for the standard CX survey types in Phase 1.
...
- **GDPR Art. 17 — right to erasure.** Phase 1's list and export endpoints inherit **survey-level** soft-delete via the existing pattern (`Survey.deletedAt IS NULL` join — `apps/api/src/routes/surveys.ts` list/detail handlers per Issue #332). `SurveyResponse` itself has no `deletedAt` column today (RFC §2.1 OQ-5 corrigendum); the erasure model is *zero-out-and-anonymize* (clear `memberId` + the AI-derived columns `sentiment / confidence / topics / summary / clusterId`), not row-soft-delete. **Forward-looking constraint on the future erasure worker**: when that worker is built (no such worker exists today — RFC §8.4), it SHALL zero those fields on every `SurveyResponse` belonging to an erased member. Phase-1 read surfaces inherit correct behavior automatically: the list endpoint renders `member: null` → `—` and the export's Member column is empty whether the worker has run yet or not.
...
- **Sentiment-band derivation:** `sentimentBandOf(float)` SHALL return `Positive | Neutral | Negative` using `SENTIMENT.POSITIVE_THRESHOLD = +0.3` and `SENTIMENT.NEGATIVE_THRESHOLD = -0.3` (RFC §10 OQ-3 corrigendum). Boundary cases (strict `<` / `>`): `+0.31 → Positive`, `+0.3 → Neutral`, `-0.3 → Neutral`, `-0.31 → Negative`, `null → null` (sentiment absent — chip hidden).
...
- **Wave + Score-band + Sentiment-band + Submitted + Channel composition**: any subset combination returns rows where every active filter holds. E.g., `scoreBands=detractor & sentimentBands=negative & wave=<batch>` returns only rows whose `score ≤ 6` AND `sentiment < -0.3` AND `distributionBatchId = <batch>` (corrigendum per RFC §10 OQ-3).
...
- **Sentiment-band correctness**: a response with `sentiment = +0.31` SHALL appear under `sentimentBands=positive`; `sentiment = +0.3` SHALL appear under `neutral`; `sentiment = -0.31` SHALL appear under `negative`; null sentiment SHALL appear under no sentiment-band filter (silently dropped). Threshold corrigendum per RFC §10 OQ-3.
