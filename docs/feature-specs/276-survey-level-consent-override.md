...
| R5 | **DEFERRED to #241.** Setting a survey to a consent mode strictly more permissive than its brand SHALL require BOTH attestation AND a non-empty reason; missing either returns 422. (Round-1 review of the RFC scoped the PATCH endpoint to #241 along with the survey-editor UX it serves; #276 ships the schema columns + the column-level VARCHAR(500) cap, which is what the #241 PATCH validation will bind to.) | Given `Brand.consentMode = EXPLICIT`, when a survey owner PATCHes `Survey.consentMode` to `IMPLIED_ON_SUBMIT` without the attestation flag OR without a `consentReason`, the API returns 422 — **owned by #241**. |
| R6 | **DEFERRED to #241.** Setting a survey to the brand's mode (or to a stricter one) SHALL NOT require attestation or reason; the API SHALL clear all three audit columns to NULL on this transition. (Same scope decision as R5.) | Given `Brand.consentMode = IMPLIED_ON_SUBMIT`, when a survey owner PATCHes `Survey.consentMode` to `EXPLICIT`, the API accepts the change with no attestation and clears the three audit columns to NULL — **owned by #241**. |
...
| R8 | **DEFERRED to #241.** The audit log SHALL capture the attesting user, timestamp, AND reason on every consent-mode override write. (Bound to the PATCH endpoint that performs the write; ships with R5/R6 in #241.) The data migration (R7) writes the same columns but does NOT go through the audit plugin — the `__migration_276__` system identifier in the column itself is the audit signal for those rows. | Given a survey with attestation set via the #241 PATCH endpoint, the audit-log row exists with `action = 'survey.update'`, `metadata.consentMode`, `metadata.consentReason`, `actorUserId` — **owned by #241**. For #276's data migration, audit attribution is via the `__migration_276__` column value. |
...
| **API integration (DEFERRED to #241)** | The PATCH survey endpoint enforces the attestation gate (R5/R6). | Owned by #241 along with the PATCH endpoint design. |
...
| **E2E (deferred to #241)** | Operator flow through the survey-editor UX (open settings panel, switch mode, attest with reason, save, see badge). | Out of scope for #276 — the survey-editor UX itself ships in #241; the API contract that #241 binds to is owned by #241 too. |
| **Compliance verification (DEFERRED to #241)** | Audit log captures the attesting user, timestamp, AND reason. | Owned by #241 along with the audit-plugin extension. The data migration's audit signal is the `__migration_276__` value in the `consentSuppressedAttestedBy` column itself. |
