...
| **DR3** | Where Survey-level overrides land: directly on `Survey` columns, a thinner `SurveyOverride` 1:1 row, or as keys inside the existing `Survey.settings` Json column. | **(a) Survey columns** — confirmed by reviewer ([r3199688368](https://github.com/mathursrus/CustomerEQ/pull/295#discussion_r3199688368): *"Directly on Survey as columns"*). R3 encodes the three typed columns on `Survey`. The other two options (SurveyOverride 1:1 row, JSON keys in `Survey.settings`) are recorded in the Alternatives table below for completeness; not pursued. |
