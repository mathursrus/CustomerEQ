...
| Default theme | ✗ (default: a neutral CustomerEQ-stock theme) | radio: 4 stock themes + "Custom (set later)" | `Brand.defaultThemeId` (NEW field, FK to existing `Theme` model) | Default survey theme, default embed widget styling, default reward-redemption page styling. Per #157, themes already exist as a CRUD entity. |
...
- Default theme picker: shows live previews so the admin sees what their member-facing surfaces will look like before committing.
