...
|---|---|---|---|
| 1 | **Identity** | `name`, `logoUrl`, `siteDomain`, `teamSize` | Name change writes through `IdentityProvider.updateOrgName()` — DB is source of truth, provider call is best-effort with retry. Logo helper text is generic ("used on customer-facing surfaces — member portals, emails, surveys"). Team size is `OrgSizeCategory` enum (1–10 / 11–50 / 51–300 / 301–5000 / 5000+ / "Prefer not to say") — optional, internal-only ("helps us optimize your experience, not shown anywhere else"). Team size lives in Identity because it is admin-profile metadata used for product-analytics / pricing-segment / engagement segmentation — not a downstream-rendering default. Field renamed from prior spec language `sizeCategory` (which was ambiguous about *whose* size); column rename is a Prisma migration in implementation scope (`Brand.sizeCategory` → `Brand.teamSize`). |
...
| 3 | **Look & Feel** | `defaultThemeId` | Theme list always shows the brand's currently-available themes. **Four default themes (Indigo, Forest, Sunset, Slate) MUST be available to every brand at first run** — admin can pick one as the default, edit any of them, or create a custom theme via deep-link to `/admin/settings/themes`. Whether the four defaults are seeded as `Theme` rows on provisioning, kept as global rows shared across all brands, or implemented some other way is a **technical-design decision deferred to RFC** (the spec's contract is "four themes available at first run, all four pickable as default"). Top action bar deep-links to `/admin/settings/themes` ("Customize the stock themes or create a new one"). Bottom helper documents downstream surfaces ("colors and fonts will be used on all customer-facing surfaces — surveys, member portals, rich-format emails"). Section name is generic to allow future Look & Feel extensions (icons, typography, branding tokens) to slot in without a rename. |
...
| F4 | `defaultThemeId` | `Brand.defaultThemeId` | #190 | Look & Feel | **Four default themes — Indigo, Forest, Sunset, Slate — MUST be available to every brand at first run.** Whether they are seeded as per-brand `Theme` rows at provisioning, kept as global rows shared across brands, or implemented some other way is a **technical-design decision deferred to RFC**. The spec's contract is "four themes available; all four pickable as default; admin can edit any of them or create a custom theme." Server returns the brand's full theme list (defaults + any custom themes) in `GET /v1/admin/brand/profile.themes`. Top action bar deep-links to `/admin/settings/themes` for "Customize the default themes or create a new one". No live preview panel on this page; preview lives on the Themes page. Selected row shows colored swatches (3 representative tokens) next to the theme name. |
| F5 | `teamSize` | `Brand.teamSize` *(`OrgSizeCategory`)* | #170 | Identity | Pill radio group with five buckets plus an opt-out: **1–10 / 11–50 / 51–300 / 301–5000 / 5000+ / "Prefer not to say"**. Buckets chosen to span SMB (1–10, 11–50), mid-market (51–300), enterprise (301–5000), and large-enterprise (5000+); the previous `201+` ceiling was rejected because it signals an SMB-only positioning that conflicts with CustomerEQ's stated mid-market-and-up ICP. Optional, internal-only ("helps us optimize your experience, not shown anywhere else"). **Column rename + enum value migration:** prior schema column was `Brand.sizeCategory` (ambiguous — whose size?), with #170-spec'd enum values `[1–10, 11–50, 51–200, 201+]`. Rename to `Brand.teamSize` and replace the enum value set with the five-bucket shape above is part of #277 implementation (Prisma column rename + enum migration, no data-row migration since #170 never shipped). Enum type name `OrgSizeCategory` unchanged. |
...
**Schema migrations in #277 scope:** rename `Brand.sizeCategory` → `Brand.teamSize` (no data change); add `Brand.timezone String @default("UTC")` and `Brand.locale String @default("en-US")`. All three are part of one Prisma migration. Beyond this, every other field already exists as a column on `Brand` after #190, #231, #170 spec work, and the original Brand model. This spec is UI + a thin profile API + a shared consent-text parser/validator/renderer package + the `SUPPORT_EMAIL` env var + the three-column migration above.
...
| R24 | The Brand schema SHALL rename `Brand.sizeCategory` to `Brand.teamSize`. The enum type `OrgSizeCategory` SHALL remain unchanged. The migration is a Prisma column rename with no data shape change. |
| R25 | Four default themes (Indigo, Forest, Sunset, Slate) SHALL be available to every brand at first run such that an admin can select any one as the default theme on the Look & Feel section without first creating a custom theme. Whether the four are seeded as per-brand `Theme` rows on provisioning, kept as global rows shared across brands, or implemented some other way is a technical-design decision; the spec's contract is that all four are pickable from first paint and editable thereafter. |
