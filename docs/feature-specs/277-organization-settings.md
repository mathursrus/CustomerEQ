...
Role gating is `admin` only — same as `/admin/settings/themes` today. Not yet using a granular permission system (no permission to grant beyond admin); when one lands (epic TBD), both the brand-level IMPLIED toggle here and the survey-level IMPLIED override shipped under #276 become candidates for a separate `legal:attest` permission split from generic admin.
