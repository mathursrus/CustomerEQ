...
| F5 | `teamSize` | `Brand.teamSize` *(`OrgSizeCategory`)* | #170 | Identity | Pill radio group: 1–10 / 11–50 / 51–200 / 201+ / "Prefer not to say". Optional, internal-only ("helps us optimize your experience, not shown anywhere else"). **Column rename:** prior schema column was `Brand.sizeCategory` (ambiguous — whose size?); rename to `Brand.teamSize` is part of #277 implementation (Prisma rename migration, no data shape change). Enum type name `OrgSizeCategory` unchanged. |
