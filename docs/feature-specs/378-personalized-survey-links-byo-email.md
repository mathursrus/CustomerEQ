...
- **`POST /v1/public/surveys/trigger` — RETIRED in V0** (Round 3 R3-15 / R3-23, decision 4a). The legacy "fire-on-event" survey trigger endpoint at `apps/api/src/routes/public.ts:602-679` is deleted in this PR. It built outbound URLs of shape `/survey/<surveyId>?email=<encoded>` (line 657) — a path that conflicts with #378's no-PII-in-URL invariant and was already half-retired (the page handler stopped reading the URL param since #241 Slice 5 per the comment at `public.ts:248`). Rather than tokenize the trigger flow (option 4a-revised), the V0 spec deletes the endpoint outright and updates the in-tree caller (the acme-coffee-demo storefront) to call the new tokenized batches API. **In-tree consequence**: `examples/acme-coffee-demo/lib/customereq.js:124` + `server.js:11` + `README.md:16,60` migrate to `POST /v1/surveys/:id/distribution-batches`. **Out-of-tree consequence**: any external integrator calling the public `/v1/public/surveys/trigger` endpoint will receive HTTP 404 after merge. The user confirmed no real external users exist at Round 3.
...
| ~~`POST` `/v1/public/surveys/trigger`~~ | **RETIRED in this PR** (R3-15 / R3-23, decision 4a). Implementation at `apps/api/src/routes/public.ts:602-679` deleted; index entry at `public.ts:133` removed. The in-tree caller `examples/acme-coffee-demo` migrates to `POST /v1/surveys/:id/distribution-batches`. External callers will receive HTTP 404 after merge. |
...
- **Invalid token error state** — load `/s/<surveyId>/r/INVALID_BYTES`; verify form renders the invalid-state copy ("This link is not valid. Please check that you copied the full link from your email, or contact the sender."); response body shape is uniform with the other error states (no token-existence leak).
...
**Update in Round 3** (R3-29 verification + R3-30 directive "Check and add"): the user authorized bundling the config update into this PR (supersedes the prior Round-1 Decision 1a, which deferred the change). The 8-vendor block is now committed alongside this spec in `fraim/config.json`. Future `feature-specification` competitor-analysis phase invocations will auto-load these competitors.
