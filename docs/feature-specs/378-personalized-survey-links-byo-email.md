...
- **Custom List** — operator supplies identifiers directly. Two sub-modes (toggleable within the card):
  - **Paste** — multi-line textarea accepting the operator's natural mail-list shape. Each entry can be (a) a bare identifier (email, phone, external_id, memberId), or (b) the RFC-822-style `Display Name <email@example.com>` form. Entries are separated by **newline, comma, or semicolon** — operators can paste a single comma-separated line copied from an email "To:" header, a one-per-line list, or any mix. Identifier kind is inferred per-entry from format. When the `Name <email>` form is used and the address resolves to an unknown identifier under auto-enroll (see below), the parsed display name populates `firstName` / `lastName` on the newly-enrolled member (last whitespace-separated token → `lastName`; everything before → `firstName`; single-token names → `firstName` only).
  - **Upload CSV** — single-column CSV. First row treated as data unless header is exactly `email` / `phone` / `external_id` / `member_id`. Multi-column CSVs (e.g., `email,firstName,lastName`) are accepted in V0 if the first column is the identifier column; remaining columns are passed through to auto-enroll as profile data when applicable. Open question OQ-S4 (below) flags one ambiguous case.
...
- **First-50-rows table**, four columns:
  ...
  3. **Last response · this survey** — most recent `SurveyResponse.completedAt` for `(memberId, surveyId)`, or `—` if none
  4. **Last response · all surveys** — most recent `SurveyResponse.completedAt` for `memberId` across all surveys, or `—` if none
  Tier is **not** a preview column in V0 (consistent with the V0 decision to drop tier-based filter predicates — Round 2 Comment R2-4). Paginates if more than 50; the page is for verification, not browsing, so 50 is the cap. For Custom List, the table groups resolved-members on top and the `unmatched` subsection below.
...
| **R6** | The Custom List card SHALL accept identifiers via paste or CSV upload. **Paste shape**: entries may be (a) a bare identifier (email / phone / external_id / memberId), or (b) RFC-822-style `Display Name <email@example.com>`. Entries are separated by **newline, comma, or semicolon** — operators may paste single-line comma-separated lists, one-per-line lists, or any mix. Identifier kind is inferred per-entry (email pattern → email; digit pattern → phone; otherwise → external_id / memberId). When the `Name <email>` form is parsed and the email resolves to an unknown identifier under auto-enroll ON, the parsed display name SHALL populate `firstName` / `lastName` on the newly-enrolled member (last whitespace-separated token → `lastName`; everything before → `firstName`; single-token names → `firstName` only). **CSV shape**: single-column CSV; first row treated as data unless header is exactly `email` / `phone` / `external_id` / `member_id`. Modes are **mutually exclusive** with Existing Members — only one mode contributes to a batch (Decision A, Round 2 R2-2). | Given the operator pastes `Jane Mitchell <jane@brand.com>, +15551234; usr_abc`, when the preview updates, then three entries are listed: Jane Mitchell with email kind, then a phone and an external_id. Given the operator pastes `Alex Chen <new@example.com>` with auto-enroll ON and `new@example.com` does not exist as a member, when Generate succeeds, then a new `Member` row exists with `firstName='Alex'`, `lastName='Chen'`, `email='new@example.com'`, `enrolledVia='BULK_DISTRIBUTION'`. Given an operator pastes a comma-separated single line of 50 entries, then all 50 are parsed independently. Given the operator pastes a single-token display name `Madonna <m@example.com>`, then auto-enroll creates the member with `firstName='Madonna'`, `lastName=NULL`. |
...
| ID | Requirement | Acceptance Criterion |
|----|-------------|-----------------------|
| **R16** | The Success state SHALL render: (a) a success banner; (b) an explanatory line immediately under the banner reading *"Users will be able to respond only once in this wave. A leaked or re-clicked link gets the 'already submitted' state."*; (c) a single **format dropdown** (options in order: `Generic` (default) / `Mailchimp` / `HubSpot` / `Klaviyo`); (d) a single **`⬇ Download CSV`** button; (e) a `Done — back to survey →` link. There SHALL be no separate "merge-tag snippet" card — the merge-tag column-naming is baked into the CSV columns per format. | Given the page is in Success state, when it renders, then the success banner is visible, the explanatory line is rendered directly below it, and exactly one dropdown + one download button + one Done link appear. |
...
- **OQ-S4 — Custom List CSV multi-column handling**: R6 admits multi-column CSV upload where extra columns are passed through to auto-enroll as profile data. Ambiguous case: when the CSV header is `email,firstName,lastName` AND the row has a `Name <email>` form *inside the first column*, which takes precedence — the parsed-from-bracket name or the explicit `firstName`/`lastName` columns? Proposed default: **explicit columns win** when present and non-empty; the bracketed name is a fallback used only when the explicit columns are absent / blank. Carries forward to RFC.
